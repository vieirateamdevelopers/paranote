...
class ArchivedNoteFragment : BaseNoteFragment() {

    override fun onActivityCreated() {
        mActionModeNote = ActionModeNote(this, true)
    }
    ...
    override fun onBindSearch(newText: String?) {
        val listArchivedNotes = mutableListOf<Note>()
        NoteApplication.noteViewModel.filterArchivedNotes(newText)
        NoteApplication.noteViewModel.notes.observe(viewLifecycleOwner, Observer {
            it?.let { notes ->
                for (note in notes) {
                    if (note.archived) {
                        listArchivedNotes.add(note)
                    }
                }
                adapter.setItems(listArchivedNotes)
            }
        })
    }
}