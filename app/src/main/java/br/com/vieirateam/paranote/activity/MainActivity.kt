...
class MainActivity : GenericActivity(R.layout.activity_main), NavigationView.OnNavigationItemSelectedListener {
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.nav_favorite -> {
                setFragment(0, "favorite", getString(R.string.nav_favorite), false, FavoriteNoteFragment())
            }
            R.id.nav_note -> {
                firstFragment()
            }
            R.id.nav_archive -> {
                setFragment(2, "archive", getString(R.string.nav_archive), false, ArchivedNoteFragment())
            }
            R.id.nav_settings -> {
                setFragment(3, "settings", getString(R.string.nav_settings), false, SettingsFragment())
            }
            R.id.nav_about -> {
                setFragment(4, "about", getString(R.string.nav_about), false, AboutFragment())
            }
            R.id.nav_exit -> {
                finishAffinity()
            }
    ...
    private fun firstFragment() {
        setFragment(1, "home", getString(R.string.nav_note), true, NoteFragment())
    }
